...
def init_speaker_encoder(source):
	...
	"""
	pretrained_state = torch.load(source, map_location='cpu')
	device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
	enc_name = source.split('/')[-1].split('\\')[-1][0:4].lower()
	if enc_name == 'camp':
		from Cam_plusplus.DTDNN import CAMPPlus
		speaker_encoder = CAMPPlus(feat_dim=80, embedding_size=192)
		speaker_encoder.load_state_dict(pretrained_state, strict = False)
	elif enc_name == 'eres':
		from eres2net.ERes2Net import ERes2Net
		speaker_encoder = ERes2Net(feat_dim=80, embedding_size=192, baseWidth=24, scale=4, expansion=4)
		speaker_encoder.load_state_dict(pretrained_state, strict = False)
	# load model
...
def main():
	...
	for rttm in tqdm.tqdm(rttms):
		...
		for i, key in enumerate(new_audio_dict):
			...
			mapping1[str(key)] = filename
			mapping[str(key)] = str(i+1)
		...
		for key in new_intervals:
			...
			if (str(key) not in mapping1.keys()):
				continue
			labels = [0] * int(length / 16000 * 25) # 40ms, one label        
			...
			if mapping1[str(key)] in good_speakers:
				res = {'filename':id_full, 'speaker_key':mapping[str(key)], 'speaker_id': speaker_id, 'labels':labels}
	...
	for file in tqdm.tqdm(files):
		if 'all.wav' not in file:
			...
			for start in range(0, wav_length - int(args.length_embedding * 16000), int(args.step_embedding * 16000)):
				...
				with torch.no_grad():
					embeddings.extend(model.forward(target_speech.unsqueeze(0).cuda()))
			if embeddings:
